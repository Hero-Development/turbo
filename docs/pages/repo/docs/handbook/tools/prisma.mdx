...
One of the most common challenges deploying Prisma from Turbo is `PrismaClientInitializationError: Prisma Client could not find its 'schema.prisma'.`

This occurs because your executable code expects `schema.prisma` (and other resources) to be found locally, but when your Prisma connection is built from another workspace, these dependencies are often omitted.  There are several ways to address this, and below we've highlighted three specific solutions.

### 1. Layers

When using AWS Lambda and/or Serverless Application Model (SAM), your dependent workspaces can be deployed as layers, and linked to your Lambda function.  
...
References:
- https://docs.aws.amazon.com/lambda/latest/dg/adding-layers.html#accessing-layer-content
